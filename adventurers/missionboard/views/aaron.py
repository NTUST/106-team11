    ...
    if request.method == 'POST':
        if CheckMissionStatus(mission):
            ...
            for i in userlist:
                ...
                User.objects.filter(username=i)[0].my_profile.missions_wip.add(mission)
                MissionApplication.objects.create(applied_by=new_user[0],mission=mission)
            ...
            for i in userlist:
                User.objects.filter(username=i)[0].my_profile.missions_completed.add(mission)
                User.objects.filter(username=i)[0].my_profile.missions_wip.remove(mission)
            ...
            userlist = userstr.split(',')[:-1]
            for i in userlist:
                User.objects.filter(username=i)[0].my_profile.missions_failed.add(mission)
                User.objects.filter(username=i)[0].my_profile.missions_wip.remove(mission)
